...
public class ReviewDAO {
	...
	//관리자 입장에서 리뷰 뽑기: 리뷰번호, 숙소이름, 작성자명, 작성자 이메일 , 리뷰제목, 리뷰내용, 작성일, 별정

	//ReviewDAO listAllReview 수정했습니다
	//<전체 리뷰 조회>관리자 입장. 모든 리뷰 목록 불러오기. 추후 검색 기능 추가 시 매개변수 필요할 듯
	   //관리자 입장에서 리뷰 뽑기: 리뷰번호, 숙소이름, 작성자명, 작성자 이메일 , 리뷰제목, 리뷰내용, 작성일, 별정
	   public ArrayList<HashMap<String, Object>> listAllReview(String category, String keyword){
	      ArrayList<HashMap<String, Object>> list = new ArrayList<HashMap<String,Object>>();
	      String sql = "select reno, p.p_name, u.u_name, u_email, re_title, re_content, re_date, r.rating  "
	            + "from review r, product p, users u "
	            + "where r.pno = p.pno and r.uno=u.uno ";
	            if(keyword!=null) { //u_email,p_name으로 검색. 
	      ...
	      try {
	         ...
	         while(rs.next()) {
	            ...
	            map.put("u_email", rs.getString(4));
	            map.put("re_title", rs.getString(5));
	            map.put("re_content", rs.getString(6));
	            map.put("re_date", rs.getString(7));
	            map.put("rating", rs.getDouble(8));
	            list.add(map);
	 ...
	 //<상품페이지 리뷰 표시 : 최신순 6개!>
		...
		//where rownum<=2;
		public ArrayList<HashMap<String, Object>> listSixReview(int pno){
			ArrayList<HashMap<String, Object>> list = new ArrayList<HashMap<String,Object>>();
